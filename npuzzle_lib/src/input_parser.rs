...
//Possible errors in parse_problems_size and parse_puzzle
pub static ERR_NUMBER_PER_LINE : &str = "Numbers per lines have to be equal to size";
pub static ERR_CHAR_INVALID : &str = "Invalid number";
...
pub static ERR_OPEN_FILE : &str = "Failed to open file";
...
fn is_puzzle_correct(puzzle : &Puzzle, size : &Atom, filename :&String) -> bool {
    let max_number : Atom = *size * *size;

    if puzzle.len() != max_number as usize {
        println!("File : {} : {}", filename, ERR_NUMBER_OF_LINES);
        return false;
    }
    if let Some(_) = puzzle.into_iter().find(|x| *x > &(max_number - 1)) {
...
fn parse_problems_size(size: &mut Atom, line : &Vec<String>) -> Option<Vec<&'static str>> {
    ...
    for word in line {
        match (word.parse::<Atom>(), had_parse_size) {
            ...
            (Err(_), false) => vec_err.push(&ERR_CHAR_INVALID),
...
fn parse_puzzle(puzzle: &mut Puzzle, size: &Atom, line : &Vec<String>)-> Option<Vec<&'static str>> {
    ...
    for word in line {
        match word.parse::<Atom>() {
            ...
            Err(_) => vec_err.push(&ERR_CHAR_INVALID),
        }
    }
...
fn parse_line(problem : &mut Problem, line : &str) -> Option<Vec<&'static str>> {
    ...
    //Parsing input
    match problem.size {
        0 => return parse_problems_size(&mut problem.size, &vec_splited),
        _ => return parse_puzzle(&mut problem.start, &problem.size, &vec_splited),
    };
}

fn generate_problem(content : &String, filename : &String) -> Option<Problem> {
    ...
    while let Some(line) = splited_file.next() {
        match parse_line(&mut problem, &line) {
            None => {},
            Some(v_err) => {
                had_error = true;
                for e in v_err {
                    println!("File : {} : at line {} : {}", filename, line, e);
                    if problem.size == 0 {
                        return None;
                    }
                }
    ...
    match is_puzzle_correct(&problem.start, &problem.size, &filename) {
        true => Some(problem),
        false => None,
    }
}

pub fn parse_from_file(file : &String) -> Option<Problem> {
    match fs::read_to_string(file) {
        Ok(s) => generate_problem(&s, &file),
        Err(_) => {
...
mod tests {
    ...
    #[test]
    fn parse_problems_size_ok() {
        let mut size : Atom = 0;
        let vec_test : Vec<String> = vec!["3".to_string()];
        let vec_err :  Option<Vec<&'static str>> = parse_problems_size(&mut size, &vec_test);
    ...
    #[test]
    fn parse_problems_size_too_small() {
        let mut size : Atom = 0;
    ...
    #[test]
    fn parse_problems_size_negative() {
        let mut size : Atom = 0;
        let vec_test : Vec<String> = vec!["-2".to_string()];
        let vec_err : Option<Vec<&'static str>> = parse_problems_size(&mut size, &vec_test);

        assert_ne!(vec_err, None);
        assert_eq!(vec_err.unwrap(), vec![ERR_CHAR_INVALID]);
        assert_eq!(size, 0);
    }

    #[test]
    fn parse_problems_size_invalid_word() {
        let mut size : Atom = 0;
    ...
    #[test]
    fn parse_problems_size_too_much_args() {
        let mut size : Atom = 0;
    ...
    fn parse_puzzle_invalid_char() {
        ...
        assert_ne!(vec_err, None);
        assert_eq!(vec_err.unwrap(), vec![ERR_CHAR_INVALID, ERR_CHAR_INVALID]);
        assert_eq!(puzzle, vec_expected);
    }

    #[test]
    fn is_puzzle_correct_ok() {
        let puzzle : Puzzle = vec![0, 1, 2, 3, 4, 5, 6, 7, 8];
        let size : Atom = 3;
        let filename : String = String::from("testfile");

        assert_eq!(true, is_puzzle_correct(&puzzle, &size, &filename));
    }

    #[test]
    fn is_puzzle_correct_line_failed_number_of_line() {
        let puzzle : Puzzle = vec![0, 1, 2, 3, 4, 5];
        let size : Atom = 3;
        let filename : String = String::from("testfile");

        assert_eq!(false, is_puzzle_correct(&puzzle, &size, &filename));
    }

    #[test]
    fn is_puzzle_correct_failed_superior_to_max_number() {
        let puzzle : Puzzle = vec![0, 1, 2, 3, 4, 5, 6, 11, 10];
        let size : Atom = 3;
        let filename : String = String::from("testfile");
    ...
    fn is_puzzle_correct_failed_duplicated_value() {
        let puzzle : Puzzle = vec![0, 1, 2, 3, 4, 3, 6, 7, 8];
        let size : Atom = 3;
        let filename : String = String::from("testfile");
